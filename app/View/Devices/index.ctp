...
?>
<p>
    <?php echo $this->Html->link('All Device', array('action' => 'index'), array('class' => 'btn btn-large')); ?>
    <?php 
        if ($this->App->isAdmin($current_user)) {
            echo $this->Html->link('Add Device', array('action' => 'add'), array('class' => 'btn btn-large'));
        }
    ?> &nbsp;&nbsp;
</p>
<div class="row-fluid">
